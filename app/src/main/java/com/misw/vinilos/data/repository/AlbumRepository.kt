...
class AlbumRepository(
    ...
    suspend fun getAlbums(): List<Album> {
        try {
            ...
            val result = withContext(dispatchers.io) { api.getAlbums() }
    ...
    suspend fun getAlbum(id: Int, refresh: Boolean = false): Album {
        try {
            ...
            val result = withContext(dispatchers.io) { api.getAlbum(id, cacheControl) }
    ...
    suspend fun createAlbum(request: AlbumRequest): Album {
        try {
            ...
            val result = withContext(dispatchers.io) { api.createAlbum(request) }
    ...
    suspend fun addTrack(albumId: Int, request: TrackRequest): Track {
        try {
            ...
            val result = withContext(dispatchers.io) { api.addTrack(albumId, request) }
